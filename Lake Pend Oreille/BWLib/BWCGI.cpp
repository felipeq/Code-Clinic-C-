...
BWCGI::BWCGI()
{
    for (BWString k : {_bwcgi_kenv_query_string, _bwcgi_kenv_gateway_interface, _bwcgi_kenv_document_root,
                       _bwcgi_kenv_reqeust_method, _bwcgi_kenv_path_info, _bwcgi_kenv_script_name,
                       _bwcgi_kenv_server_software, _bwcgi_kenv_content_type, _bwcgi_kenv_user_agent,
                       _bwcgi_kenv_cookie})
    {
        char *s = std::getenv(k);
        if (s && *s) _env[k] = s;
