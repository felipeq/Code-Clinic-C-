...
long BWLPO::count(const BWString &type, const BWString &date) const
{
    ...
    BWString rs = db->get_query_value("SELECT COUNT(*) FROM " + _lpo_name + " WHERE type = ? AND stamp LIKE ?",
                                      {type, datelike});
...
bool BWLPO::add_line(const BWString &line, const BWString &type)
{
    ...
    BWString rs = db->get_query_value("SELECT value FROM " + _lpo_name + " WHERE type = ? AND stamp = ?",
                                      {type, SQLdate});
    if (rs.have_value())
    {
        db->do_query("UPDATE " + _lpo_name + " SET value = ? WHERE type = ? AND stamp = ?",
                     {lineparts[_iDATA], type, SQLdate});
    } else
    {
        db->do_query("INSERT INTO " + _lpo_name + " (value, type, stamp) VALUES (?, ?, ?)",
                     {lineparts[_iDATA], type, SQLdate});
    }
...
BWLPO::lporesult BWLPO::get_mean(const BWString &type, const BWString &date) const
{
    ...
    BWString rs = db->get_query_value("SELECT AVG(value) FROM " + _lpo_name + " WHERE type = ? AND stamp LIKE ?",
                                      {type, date_like});
...
BWLPO::lporesult BWLPO::get_median(const BWString &type, const BWString &date) const
{
    ...
    if (nrecs < 3)
    {    // 0, 1, or 2 - mean is same as median
    ...
    {        // odd - get the middle value
        ...
        BWString rs = db->get_query_value(
                "SELECT value FROM " + _lpo_name + " WHERE type = ? AND stamp LIKE ? ORDER BY value LIMIT 1 OFFSET ?",
                {type, date_like, nstring});
        if (rs.have_value())
        {
            rv.ivalue = atol(rs.c_str());
            rv.dvalue = atof(rs.c_str());
    ...
    {            // even - get the mean of the middle two values
        ...
        db->prepare_query(
                "SELECT value FROM " + _lpo_name + " WHERE type = ? AND stamp LIKE ? ORDER BY value LIMIT 2 OFFSET ?",
                {type, date_like, nstring});
